  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        const normalizedLocations = data.features.map(normalizeCategory);
  ...
  const handleFavoriteClick = async (pinId: string) => {
    ...
    try {
      ...
      if (error instanceof Error && error.message === 'User not authenticated') {
        showMessage('Please log in to manage favorites');
      } else {
        ...
        <div class="grid grid-cols-2 gap-2">
          ${url ? `
            <a
              ...
              class="block"
            >
              <button class="w-full px-3 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors ${isMobile ? 'text-xs' : 'text-sm'} font-medium">
